#' @name linearregSGD
#' @title parameter estimation and confidence interval construction for linear regression with SGD
...
#' @details Parameter estimation and confidence interval construction are based on Example 1 of Yixin Fang, Jinfeng Xu, and Lei Yang.
...
linearSGD<-function(X,Y,lr0, C=2/3, burnin=0.2){
...
linearSGDCI<-function(X, Y, lr0, C=2/3, burnin=0.2, B=200, per_dis='Poi', CI.level=0.95, CI.method='sd'){
  # X is the design matrix, covariates, it does not contain the
  # column of 1s
  # y is the dependent variable, dimension: p*1
