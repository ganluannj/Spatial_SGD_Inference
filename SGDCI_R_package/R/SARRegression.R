...
Update2<-function (Par, Wdialistj, Neighmean, littlex, littley, LR, j, K=K, fac){
  # par is a vector with length p+4
  # par[1:p] is betaest
  # par[p+1] is sigsqest
  # par[p+2] is rhoest
  ...
  InvAjj = InvAdig2(rholist, Wdialistj)
  ...
  # littlex=X[j,]
  # littley=Y[j]
  sqpart=(littley - rhoest*Neighmean-sum(littlex*betaest))[1,1]
  derbeta= sqpart*littlex/sigsqest
  betaesttemp=betaest+LR*derbeta*fac
...
Updateper<-function(ParCI, W, Wdialist, X, Y, LR, j, K=K, faclist){
  ...
  littlex=X[j,]
  littley=Y[j]
  Wdialistj=rep(0, length(Wdialist))
  Neighmean=W[j,]%*%Y
  for (i in 1: length(Wdialist)){
    Wdialistj[i]=Wdialist[[i]][j]
  }
  for (i in 1:Q){
    ParCI2[(k*(i-1)+1):(k*i)]=Update2(Par=ParCI[(k*(i-1)+1):(k*i)], Wdialistj=Wdialistj, littlex=littlex,
                                     littley=littley, Neighmean=Neighmean,LR=LR, j=j, K=K, fac=faclist[i])
  }
  return (ParCI2)
}
...
InvAdig2<-function(Rholist, Wdialistj){
  return (sum(Rholist*Wdialistj))
}
...
W_=(t(ln)%*%ln-diag(N))/(N-1)
W=Neigb(N**0.5)

#############################################################
###### Constructing W for each data points have #############
###### exactly 2 neighbors                      #############
#############################################################
Neigb2<-function(N){
  # N is the total number of data points
  library(Matrix)
  W=Matrix(nrow=N,ncol=N,data=0,sparse=TRUE)
  W[1,2]=W[1,N]=W[N,N-1]=W[N,1]=0.5
  for (i in 2:(N-1)){
    W[i,i-1]=W[i,i+1]=0.5
  }
  return (W)
...
#############################################################
Neigb1<-function(N){
  # N is the total number of data points, N has to be a even number
  # 1-2 neighbors; 3-4 neighbors; 5-6 neighbors; ... ; (N-1)-N neighbors
  library(Matrix)
  W=Matrix(nrow=N,ncol=N,data=0,sparse=TRUE)
  K=as.integer(N/2)
  for (i in 1:K){
    W[2*i-1, 2*i]=W[2*i, 2*i-1]=1
  }
  return (W)
}
