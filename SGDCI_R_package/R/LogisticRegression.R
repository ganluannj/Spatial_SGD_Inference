#' @name logisticregSGD
#' @title parameter estimation and confidence interval construction for logistic regression with SGD
#' @description This for applying stochastic gradient descent (SGD) for parameter estimation of
#' logistic regression model. \code{LogiSGD} apply SGD for parameter estimation and \code{LogiSGDCI}
#' apply SGD for parameter estimation and also construct confidence intervals for estimates.
#' @details Parameter estimation and confidence interval construction are based on Example 2 of Yixin Fang, Jinfeng Xu, and Lei Yang.
...
#' @param y response variable, it takes values of 1 or -1
...
#'
#' @return
#' \strong{\code{LogiSGD}} \cr
#' This function will return estimated coefficients for logistic regression. \cr
#' \strong{\code{LogiSGDCI}} \cr
#' This function will return estimated coefficients and confidence intervals for logistic regression. \cr
#'
#'
#' @examples
#' \dontrun{
...
#' glm function provided by r as a control
#' glm(yGLM ~ x1 + x2, family='binomial'(link='logit'))
#' }
#'
...
LogiSGD<-function(X,Y,lr0, C=2/3, burnin=0.2){
...
#' @rdname logisticregSGD
#' @export

LogiSGDCI<-function(X, Y, lr0, C=2/3, burnin=0.2, B=200, per_dis='Poi', CI.level=0.95, CI.method='sd'){
  # X is the design matrix, covariates, it does not contain the
  # column of 1s
  # y is the dependent variable, it takes values of 1 and -1, dimension: p*1
  ...
  for (i in 1:N){
    ...
    EXP = exp(y_xbeta) # 1*(B+1)
    Delta= x %*% (y/(1+EXP)) # p*(B+1)
    if (per_dis=='Poi'){
